...
- **M1 durumu (02 kabul kriterleri)**: ✓ yıl göstergesi, ✓ detay + geri tuşu, ✓ `/admin` anonim 302, ✓ kaydet → anında, ✓ taslak RLS ile gizli (`rls-proof.sh`), ✓ 4 dil rotası + "bu dilde yok" rozeti. ✗ **50 yayınlı olay**: bulutta 28 yayın + 15 taslak (kullanıcı yayınlar → 43); +7 Aydınlanma olayı gerekir. ✓ **Lighthouse mobil 85+**: canlı timeline 93 (2026-09-03); olay sayfası canlıda 80'di (dinamik render) → `generateStaticParams` ile 112 sayfa prerender, yerel üretimde timeline 100 / olay 100, CLS 0.139 → 0.021 (filtre çipi iskeleti). Merge sonrası canlıda yeniden ölç.
