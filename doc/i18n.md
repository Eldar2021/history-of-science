...
birden yazılır, sonradan tamamlanmaz.

Claude API'ye verilenler: alanlar (`title, summary, body, why_it_matters, if_you_were_there, place_name`),
ses tonu kuralları (`icerik.md`), terim sözlüğü, yerleşik isim yazımları, hedef dil; **ky için ru+tr
referans**. Yıl ve sayı çevrilmez (formatlama sitede). Gövdedeki Markdown yapısı korunur: figür adresleri
ve künyeleri, kutu anahtar sözcükleri (`[!NOTE]` — İngilizce kalır, içi çevrilir) ve `$...$` matematiği
olduğu gibi taşınır. Çıktı JSON şemayla doğrulanır, boş alan gelirse kaydedilmez. Her çeviri
`status='machine'`; admin düzeltince `reviewed`. Rapor: `check-i18n.ts` (eksik anahtar / eksik dil).
