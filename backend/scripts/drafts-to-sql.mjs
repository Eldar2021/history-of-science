 ...
 *
 * Writes events as status='draft' only (ADR-014: publishing is a human action). Links whose target
 * does not exist yet (slugs ending in "?") are kept in research_note instead of event_links.
 */
...
from events where slug = ${q(d.slug)}
...
delete from event_disciplines where event_id = (select id from events where slug = ${q(d.slug)});
...
where e.slug = ${q(d.slug)} and di.slug = any (${arr(d.disciplines)});

delete from sources where event_id = (select id from events where slug = ${q(d.slug)});`);
  for (const s of d.sources) {
    out.push(`insert into sources (event_id, title, url, kind) select id, ${q(s.title)}, ${q(s.url)}, ${q(s.kind)} from events where slug = ${q(d.slug)};`);
  }
}
