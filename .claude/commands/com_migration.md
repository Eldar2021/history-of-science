...
1. Read `doc/mimari.md` (data model section) and list the existing files in `backend/supabase/migrations/` to find the next number (`NNNN_snake_case.sql`).
...
4. Update the data-model table in `doc/mimari.md` so it matches the migration. Keep it a summary; the SQL files are the source of truth.
